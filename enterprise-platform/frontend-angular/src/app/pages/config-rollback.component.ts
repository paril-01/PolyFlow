...
@Component({
  selector: 'app-config-rollback',
  template: `
    <div class="p-4 bg-white rounded shadow-sm border">
      <h3 class="text-lg font-semibold">Config Rollback</h3>
...
export class ConfigRollbackComponent {
  loading = false;
  result: any = null;
  ...
  execute() {
    this.loading = true;
    this.error = null;
    this.http.post('/api/v1/admin/config-rollback', {}).subscribe({

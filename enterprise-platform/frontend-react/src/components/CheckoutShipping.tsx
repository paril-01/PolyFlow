...
export const CheckoutShipping: React.FC<CheckoutShippingProps> = ({ onComplete }) => {
  ...
  const handleCheckoutShipping = async () => {
    setLoading(true);
    setError(null);
    try {
      const response = await api.post('/frontend_customer/checkout-shipping');
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-sm border">
      <h3 className="text-lg font-semibold text-gray-800">Checkout Shipping</h3>
      <button

...
export const SearchAutocomplete: React.FC<SearchAutocompleteProps> = ({ onComplete }) => {
  ...
  const handleSearchAutocomplete = async () => {
    ...
    setError(null);
    try {
      const response = await api.post('/frontend_customer/search-autocomplete');
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-sm border">
      <h3 className="text-lg font-semibold text-gray-800">Search Autocomplete</h3>
      <button

...
export const CheckoutConfirm: React.FC<CheckoutConfirmProps> = ({ onComplete }) => {
  ...
  const handleCheckoutConfirm = async () => {
    ...
    setError(null);
    try {
      const response = await api.post('/frontend_customer/checkout-confirm');
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-sm border">
      <h3 className="text-lg font-semibold text-gray-800">Checkout Confirm</h3>
      <button

...
export const WishlistAdd: React.FC<WishlistAddProps> = ({ onComplete }) => {
  ...
  const handleWishlistAdd = async () => {
    ...
    setError(null);
    try {
      const response = await api.post('/frontend_customer/wishlist-add');
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-sm border">
      <h3 className="text-lg font-semibold text-gray-800">Wishlist Add</h3>
      <button

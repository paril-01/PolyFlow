...
export const FilterCategory: React.FC<FilterCategoryProps> = ({ onComplete }) => {
  ...
  const handleFilterCategory = async () => {
    ...
    setError(null);
    try {
      const response = await api.post('/frontend_customer/filter-category');
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-sm border">
      <h3 className="text-lg font-semibold text-gray-800">Filter Category</h3>
      <button

...
export const FilterBrand: React.FC<FilterBrandProps> = ({ onComplete }) => {
  ...
  const handleFilterBrand = async () => {
    ...
    setError(null);
    try {
      const response = await api.post('/frontend_customer/filter-brand');
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-sm border">
      <h3 className="text-lg font-semibold text-gray-800">Filter Brand</h3>
      <button

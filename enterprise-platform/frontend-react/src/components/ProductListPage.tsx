...
export const ProductListPage: React.FC<ProductListPageProps> = ({ onComplete }) => {
  ...
  const handleProductListPage = async () => {
    ...
    setError(null);
    try {
      const response = await api.post('/frontend_customer/product-list-page');
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-sm border">
      <h3 className="text-lg font-semibold text-gray-800">Product List Page</h3>
      <button

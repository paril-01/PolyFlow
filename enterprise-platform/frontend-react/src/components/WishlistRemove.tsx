...
export const WishlistRemove: React.FC<WishlistRemoveProps> = ({ onComplete }) => {
  ...
  const handleWishlistRemove = async () => {
    ...
    setError(null);
    try {
      const response = await api.post('/frontend_customer/wishlist-remove');
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-sm border">
      <h3 className="text-lg font-semibold text-gray-800">Wishlist Remove</h3>
      <button

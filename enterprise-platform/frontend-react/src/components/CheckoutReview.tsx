...
export const CheckoutReview: React.FC<CheckoutReviewProps> = ({ onComplete }) => {
  ...
  const handleCheckoutReview = async () => {
    ...
    setError(null);
    try {
      const response = await api.post('/frontend_customer/checkout-review');
  ...
  return (
    <div className="p-4 bg-white rounded-lg shadow-sm border">
      <h3 className="text-lg font-semibold text-gray-800">Checkout Review</h3>
      <button

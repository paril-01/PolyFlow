...
public class RazorpayRefundService {
    ...
    private static final Logger log = LoggerFactory.getLogger(RazorpayRefundService.class);

    public Map<String, Object> execute(Map<String, Object> request) {
        long start = System.currentTimeMillis();
        String traceId = UUID.randomUUID().toString().substring(0, 8);
        log.info("[{}] Executing razorpay_refund", traceId);
        ...
        try {
            ...
            log.error("[{}] razorpay_refund failed: {}", traceId, e.getMessage());
